...
void checkResult(bool isCreation, std::vector<OpenchatTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    OpenchatTxConverter converter(transaction, NULL);
    ExtractOpenchatTX openchatTx;
    BOOST_CHECK(converter.extractionOpenchatTransactions(openchatTx));
    std::vector<OpenchatTransaction> result = openchatTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(openchattxconverter_tests, TestingSetup)
