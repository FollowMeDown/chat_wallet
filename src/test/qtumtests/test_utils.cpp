...
void initState(){
    ...
    const std::string dirOpenchat = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<OpenchatState>(new OpenchatState(dev::u256(0), OpenchatState::openDB(dirOpenchat, hashDB, dev::WithExisting::Trust), dirOpenchat + "/openchatDB", dev::eth::BaseState::Empty));
...
OpenchatTransaction createOpenchatTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    OpenchatTransaction txEth;
    if(recipient == dev::Address()){
        txEth = OpenchatTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = OpenchatTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<OpenchatTransaction> txs){
