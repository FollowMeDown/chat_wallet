#include "openchatDGP.h"

void OpenchatDGP::initDataEIP158(){
...
bool OpenchatDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule OpenchatDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t OpenchatDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t OpenchatDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t OpenchatDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t OpenchatDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool OpenchatDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void OpenchatDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void OpenchatDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void OpenchatDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void OpenchatDGP::createParamsInstance(){
...
dev::Address OpenchatDGP::getAddressForBlock(unsigned int blockHeight){
...
void OpenchatDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OpenchatDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OpenchatDGP::parseStorageOneUint64(uint64_t& value){
...
void OpenchatDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule OpenchatDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
