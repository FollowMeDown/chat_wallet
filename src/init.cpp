...
void Shutdown()
{
    ...
    if (pwalletMain)
    {
        StakeOpenchats(false, pwalletMain);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/openchatproject/chat>";
    const std::string URL_WEBSITE = "<https://openchat.org>";
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                boost::filesystem::path openchatStateDir = GetDataDir() / "stateOpenchat";
                bool fStatus = boost::filesystem::exists(openchatStateDir);
                const std::string dirOpenchat(openchatStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsOpenchatstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<OpenchatState>(new OpenchatState(dev::u256(0), OpenchatState::openDB(dirOpenchat, hashDB, dev::WithExisting::Trust), dirOpenchat, existsOpenchatstate));
    ...
    if (!GetBoolArg("-staking", DEFAULT_STAKE))
        ...
        StakeOpenchats(true, pwalletMain);
#endif
    // ********************************************************* Step 12: finished
