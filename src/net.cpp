...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "openchat " + FormatFullVersion();
...
void CConnman::ProcessOneShot()
{
    ...
    if (grant) {
        if (!OpenchatConnection(addr, false, &grant, strDest.c_str(), true))
            AddOneShot(strDest);
    }
}
...
void CConnman::ThreadOpenConnections()
{
    ...
    if (mapMultiArgs.count("-connect") && mapMultiArgs.at("-connect").size() > 0)
    {
        for (int64_t nLoop = 0;; nLoop++)
        {
            ...
            BOOST_FOREACH(const std::string& strAddr, mapMultiArgs.at("-connect"))
            {
                ...
                OpenchatConnection(addr, false, NULL, strAddr.c_str());
    ...
    while (!interruptNet)
    {
        ...
        if (addrConnect.IsValid()) {
            ...
            OpenchatConnection(addrConnect, (int)setConnected.size() >= std::min(nMaxConnections - 1, 2), &grant, NULL, false, fFeeler);
        }
    }
}
    ...
    while (true)
    {
        ...
        for (const AddedNodeInfo& info : vInfo) {
            if (!info.fConnected) {
                ...
                // OpenchatConnection can detect existing connections to that IP/port.
                tried = true;
                CService service(LookupNumeric(info.strAddedNode.c_str(), Params().GetDefaultPort()));
                OpenchatConnection(CAddress(service, NODE_NONE), false, &grant, info.strAddedNode.c_str(), false, false, true);
                if (!interruptNet.sleep_for(std::chrono::milliseconds(500)))
                    return;
            }
        }
...
bool CConnman::OpenchatConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound, const char *pszDest, bool fOneShot, bool fFeeler, bool fAddnode)
{
    //
