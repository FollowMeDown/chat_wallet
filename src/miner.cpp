...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setOpenchatSchedule(openchatDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = openchatDGP.getBlockSize(nHeight);
    minGasPrice = openchatDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = openchatDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    OpenchatTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractOpenchatTX resultConverter;
    if(!convert.extractionOpenchatTransactions(resultConverter)){
    ...
    for(OpenchatTransaction openchatTransaction : openchatTransactions){
        txGas += openchatTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + openchatTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(openchatTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, openchatTransactions, hardBlockGasLimit);
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("openchatcoin-miner");
...
void StakeOpenchats(bool fStake, CWallet *pwallet)
{
