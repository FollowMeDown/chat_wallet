...
bool ToHash160(const std::string& strOpenchatAddress, std::string& strHash160)
{
    CBitcoinAddress openchatAddress(strOpenchatAddress);
    if(openchatAddress.IsValid()){
        CKeyID keyid;
        openchatAddress.GetKeyID(keyid);
...
bool ToOpenchatAddress(const std::string& strHash160, std::string& strOpenchatAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress openchatAddress;
    openchatAddress.Set(keyid);
    if(openchatAddress.IsValid()){
        strOpenchatAddress = openchatAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToOpenchatAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToOpenchatAddress(tokenEvent.receiver, tokenEvent.receiver);
