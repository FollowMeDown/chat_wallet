...
#include "openchatstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

OpenchatState::OpenchatState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = OpenchatState::openDB(_path + "/openchatDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

OpenchatState::OpenchatState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute OpenchatState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, OpenchatTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createOpenchatAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            openchat::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> OpenchatState::vins() const // temp
{
...
void OpenchatState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* OpenchatState::vin(dev::Address const& _a) const
{
    return const_cast<OpenchatState*>(this)->vin(_a);
}

Vin* OpenchatState::vin(dev::Address const& _addr)
{
...
// void OpenchatState::commit(CommitBehaviour _commitBehaviour)
// {
...
void OpenchatState::kill(dev::Address _addr)
{
...
void OpenchatState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address OpenchatState::createOpenchatAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void OpenchatState::deleteAccounts(std::set<dev::Address>& addrs){
...
void OpenchatState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void OpenchatState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
