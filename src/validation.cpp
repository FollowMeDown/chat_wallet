...
#if defined(NDEBUG)
# error "openchat cannot be compiled without assertions."
#endif
...
std::unique_ptr<OpenchatState> globalState;
...
const std::string strMessageMagic = "openchat Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // openchat
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            OpenchatDGP openchatDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = openchatDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = openchatDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            OpenchatTxConverter converter(tx, NULL);
            ExtractOpenchatTX resultConverter;
            if(!converter.extractionOpenchatTransactions(resultConverter)){
            ...
            std::vector<OpenchatTransaction> openchatTransactions = resultConverter.first;
            std::vector<EthTransactionParams> openchatETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(OpenchatTransaction openchatTransaction : openchatTransactions){
                sumGas += openchatTransaction.gas() * openchatTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, openchatTransaction.gasPrice());
                } else {
                    txMinGasPrice = openchatTransaction.gasPrice();
                }
                VersionVM v = openchatTransaction.getVersion();
                if(v.format!=0)
                ...
                if(openchatTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(openchatTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += openchatTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)openchatTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(openchatETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > openchatTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s openchat additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // openchat
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // openchat

    if(pfClean == NULL && fLogEvents){
        boost::filesystem::path stateDir = GetDataDir() / "stateOpenchat";
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    OpenchatTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<OpenchatTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path openchatDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(openchatDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(openchatDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(OpenchatTransaction& tx : txs){
        //validate VM version
...
bool OpenchatTxConverter::extractionOpenchatTransactions(ExtractOpenchatTX& openchattx){
    std::vector<OpenchatTransaction> resultTX;
    ...
    openchattx = std::make_pair(resultTX, resultETP);
    return true;
}

bool OpenchatTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool OpenchatTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
OpenchatTransaction OpenchatTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    OpenchatTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = OpenchatTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = OpenchatTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // openchat
    OpenchatDGP openchatDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setOpenchatSchedule(openchatDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = openchatDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = openchatDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = openchatDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // openchat
...
///////////////////////////////////////////////////////////////////////////////////////// openchat
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            OpenchatTxConverter convert(tx, &view, &block.vtx);

            ExtractOpenchatTX resultConvertOpenchatTX;
            if(!convert.extractionOpenchatTransactions(resultConvertOpenchatTX)){
            ...
            if(!CheckMinGasPrice(resultConvertOpenchatTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertOpenchatTX.first, blockGasLimit);
            ...
            for(OpenchatTransaction& qtx : resultConvertOpenchatTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertOpenchatTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertOpenchatTX.first[k].from(), resultConvertOpenchatTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // openchat
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // openchat
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // openchat
            globalState->setRootUTXO(oldHashUTXORoot); // openchat
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // openchat
        uint32_t sizeBlockDGP = openchatDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for openchat PoS we need to sync this after every block to ensure txdb is populated for
